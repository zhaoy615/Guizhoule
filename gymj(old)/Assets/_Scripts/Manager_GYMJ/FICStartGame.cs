...
public class FICStartGame : MonoBehaviour
{
    ...
    public GameObject[] huSkillGOArray = new GameObject[7];//不同胡特效数组
    ...
    private void InitArrays()
    {
        ...
        huSkillGOArray[6] = Resources.Load<GameObject>("Game_GYMJ/Prefabs/texiao/item_fpnew");
    ...
    void ShowHuPaiBeforeJI(int hFW, int hType, MaJiang hMJ, int fpFW = 0)
    {
        if (hFW != fpFW)
        {
            ...
            if (fpFW != 0)
            {
                fw = GameInfo.GetFW(fpFW);
                switch (fw)
                {
                    case FW.East:
                        GameObject go = Instantiate(huSkillGOArray[6], AniJi_E.transform.position, Quaternion.identity, transform.Find("/Game_UI/Fixed_UI")) as GameObject;
                        StartCoroutine(Move(hFW, go));
                        break;
                    case FW.West:
                        go = GameObject.Instantiate(huSkillGOArray[6], AniJi_W.transform.position, Quaternion.identity, transform.Find("/Game_UI/Fixed_UI")) as GameObject;
                        StartCoroutine(Move(hFW, go));
                        break;
                    case FW.North:
                        go = GameObject.Instantiate(huSkillGOArray[6], AniJi_N.transform.position, Quaternion.identity, transform.Find("/Game_UI/Fixed_UI")) as GameObject;
                        StartCoroutine(Move(hFW, go));
                        break;
                    case FW.South:
                        go = GameObject.Instantiate(huSkillGOArray[6], AniJi_S.transform.position, Quaternion.identity, transform.Find("/Game_UI/Fixed_UI")) as GameObject;
                        StartCoroutine(Move(hFW, go));
                        break;
                }
            }
    ...
    public IEnumerator Move(int hFW,GameObject go)
    {
        yield return new WaitForSeconds(1.5f);
     
        go.transform.DOLocalMove(managerGame.GetHeadPos((int)GameInfo.GetFW(hFW)), 1.5f);
        yield return new WaitForSeconds(0.5f);
        go.GetComponent<DeskSkillDestory>().enabled = true;
    }
    /// <summary>
    /// 服务器下发定缺消息
    /// </summary>
    ...
    private void ShowOneJieSuan(UserJS item)
    {
        ...
        for (int i = 0; i < jpList.Count; i++)
        {
            ...
            if (jpList[i].PaiHS > 0 && jpList[i].type != 11)
            {
                ...
                else if(jpList[i].PaiHS==103)
                {
                    jiGO.transform.Find("text_ji_type").GetComponent<Text>().text = "杀报";
                }else
                {
