...
namespace MJBLL.mjrule
{
    ...
    public class WXLogin : ICommand<GameSession, ProtobufRequestInfo>
    {
        ...
        public void ExecuteCommand(GameSession session, ProtobufRequestInfo requestInfo)
        {
            ...
            session.Logger.Debug("登录用户openid:" + userinfo.Openid + "|昵称:" + userinfo.Nickname + "----------" + DateTime.Now + "|userid : " + usermodel.id + " | sessionid ：" + session.SessionID) ;
            ...
            if (olduser == null)
            {
                ...
                {
                    session.Logger.Debug(" WXLogin : 非法连接");
                    //session.Close();
                    return;
                }
                ReturnLogin log = ReturnLogin.CreateBuilder().SetLoginstat(1).SetUserID(int.Parse(usermodel.id.ToString())).SetUserRoomCard(150).Build();
            ...
            else
            {
                ...
                {
                    ReturnLogin log = ReturnLogin.CreateBuilder().SetLoginstat(2).SetUserID(int.Parse(usermodel.id.ToString())).SetUserRoomCard(105).Build();
                    byte[] msg = log.ToByteArray();
                    ...
                    Console.WriteLine("登录游戏失败,当前对象存在----------" + DateTime.Now + " , sssionID--------" + session.SessionID);
                }
                else
                {
