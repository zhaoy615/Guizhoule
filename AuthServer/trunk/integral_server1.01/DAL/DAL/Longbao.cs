...
namespace DAL.DAL
{
    public class Longbao
    {
        ...
        {
            using (var conn = new SqlConnection(PubConstant.LongBaoConnStr))
            {
            ...
            using (var conn = new SqlConnection(PubConstant.LongBaoConnStr))
            {
                ...
                try
                {
                    ...
                    string strLog = @"INSERT INTO [PayList] (UID,InOut, PayType, FromTo,Val,Remark,Status,CreateTime,UpdateTime,PriceType)
                            VALUES ( @id,0 ,2,10,@count,@logging,1,GETDATE(),GETDATE(),0) ";
        ...
        public int AddLongBao(string id, int count, string logging)
        {
            using (var conn = new SqlConnection(PubConstant.LongBaoConnStr))
            {
                ...
                try
                {
                    ...
                    string strLog = @"INSERT INTO [PayList] (UID,InOut, PayType, FromTo,Val,Remark,Status,CreateTime,UpdateTime,PriceType)
                            VALUES ( @id,1 ,2,10,@count,@logging,1,GETDATE(),GETDATE(),0) ";
                    i = conn.Execute(str, new { id = id, count = count }, transaction);
                    if (i != 0)
                        conn.Execute(strLog, new { id = id, logging = logging, count = count }, transaction);
                    transaction.Commit();
        ...
        public int GetLongBaoCount(string ID)
        {
            using (var conn = new SqlConnection(PubConstant.LongBaoConnStr))
            {
                string str = @"select Score from Users where ID=@ID";
                return conn.QueryFirstOrDefault<int>(str, new { ID = ID });
            }
        }
